function [c_mean_UDR, Dc_mean_UDR, c_STD_UDR, Dc_STD_UDR, c_mean_MC, Dc_mean_MC, c_STD_MC, Dc_STD_MC] = ...
        udr3D(mesh_info3D, rand_numbr_f, eole_order_e, GH_nodes, GH_weights, u_handle, n_monte_carlo)
    ...
    n_rand_var = rand_numbr_f + eole_order_e;
    ...
    response_sensitivity_Dgi = zeros(nely, nelx, nelz, n_quad_nodes);

    % Temp variable storing g_i*Dg_i/Drho_e
    temp_gi_times_Dgi = zeros(nely, nelx, nelz, n_quad_nodes);

    % Temp variable storing E[gi]*E[Dgi]
    temp_Egi_times_EDgi = zeros(nely, nelx, nelz, n_rand_var);
    ...
    for ii = 1:n_rand_var
        ...
        E_Dgi = sum(response_sensitivity_Dgi(:, :, :, (ii - 1) * n_quad_nodes + 1:ii * n_quad_nodes), 4);

        temp_Egi_times_EDgi(:, :, :, ii) = 2 * E_gi * E_Dgi;
    end
    ...
    Dc_mean_UDR = sum(response_sensitivity_Dgi, 4) - (n_rand_var - 1) * Dc_mean;
    ...
    Dc_STD_UDR = 1/2 * (sum(response_gi_square) - response_square_mean_temp)^(-1/2) * ...
        (sum(temp_gi_times_Dgi, 4) - sum(temp_Egi_times_EDgi, 4));

    %% Quadrature by Monte Carlo method
    if n_monte_carlo > 0
        %% Initialization
        response_MC_temp = zeros(n_monte_carlo, 1);
        sensitivity_MC_temp = zeros(nely, nelx, nelz, n_monte_carlo);

        c_times_Dc = zeros(nely, nelx, nelz, n_monte_carlo);
        ...
        Dc_mean_MC = mean(sensitivity_MC_temp, 4);

        %     std_response_MC = std(response_MC_temp, 0, 1);
        c_STD_MC = std(response_MC_temp);
        Dc_STD_MC = 1 / c_STD_MC * (mean(c_times_Dc, 4) - c_mean_MC * Dc_mean_MC);
