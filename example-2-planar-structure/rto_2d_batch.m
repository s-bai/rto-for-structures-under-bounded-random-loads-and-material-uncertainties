function rto_2d_batch(nelx, nely, volfrac, penalty, rmin, k_level, n_tp_grid, n_MC_samples, kk)
    ...
    max_iter_step = 500; % Maximum iteration steps
    max_change = 0.001;

    %% Start recording log file
    clc;
    file_index = num2str(kk);
    cmd_file_name = strcat('rto_2d', file_index, '.log');

    cmd_name = 'rto_2d';
    cmd_parameters = [nelx, nely, volfrac, penalty, rmin, k_level, n_tp_grid, kk];
    diary (cmd_file_name);
    cmd_output = cmd_history(cmd_name, cmd_parameters);

    %% Load parameters from file
    datafile_prefix = 'rto_2d';
    datafile_suffix = '.dat';
    datafile_name = strcat(datafile_prefix, file_index, datafile_suffix);

    file_ID = fopen(datafile_name, 'r'); % Open file
    ...
    %% ----------------- Loads for two-side mid-hinged beam under two loads -----------------
    % Loads #1
    f_nominal(:, :, 1) = [
                    (nely + 1) * nelx / 2 + nely + 1, ellips_char_parameter(1).fx_nominal;
                    (nely + 1) * nelx / 2 + nely + 2, ellips_char_parameter(1).fy_nominal];
    ...
    % % Loads #2
    f_nominal(:, :, 2) = [
                    (nely + 1) * nelx * 3/2 + nely + 1, ellips_char_parameter(2).fx_nominal;
                    (nely + 1) * nelx * 3/2 + nely + 2, ellips_char_parameter(2).fy_nominal];
    ...
    for ii = 1:n_ellipse
        w(ii).w_matrix_physical = ellips_char_parameter(ii).w;

        [w(ii).Q_physical, w(ii).SIGMA_physical] = eig(w(ii).w_matrix_physical);

        w(ii).w_matrix_interim = w(ii).SIGMA_physical;

        [w(ii).Q_interim, w(ii).SIGMA_interim] = eig(w(ii).w_matrix_interim);

        w(ii).T_interim = w(ii).Q_interim * sqrt(w(ii).SIGMA_interim) * w(ii).Q_interim.';

        w(ii).T_interim_inverse = inv(w(ii).T_interim);

        w(ii).T_physical = w(ii).Q_physical * sqrt(w(ii).SIGMA_physical) * w(ii).Q_physical.';

        w(ii).T_physical_inverse = inv(w(ii).T_physical);
    ...
    rho_history(:, :, 1) = rho_Phys; % Initial design as the first step
    ...
    %% Start optimization
    figure_prefix = 'Topology layout evolution';
    figure_title = strcat(figure_prefix, {' '}, file_index);
    figure('Name', figure_title{1}, 'NumberTitle', 'off');

    while (change > max_change) && (iter <= max_iter_step)

        fprintf('\n------------------------ Iter. %d ------------------------\n', iter);

        %% Increase penalty factor by 0.25 every 20 steps after the 100th step, and the maximum penalty factor is 6
        if iter > 100 && mod(iter, 20) == 0
        ...
        FEA_handle = @(xi_all) FEA(mesh_info, e, problem_parameters.eole_order_e, ...
            ...
            f_components, rand_numbr_f, rho_Phys, penalty, w, ...
        ...
        if problem_parameters.eole_order_t > 0
            ...
            volume_handle = @(xi_all) total_volume(mesh_info, t, rho_Phys, ...
        ...
        if problem_parameters.eole_order_t == 0
            ...
            rto_constraint = sum(sum(rho_Phys)) - nelx * nely * volfrac;
            Drto_constraint = ones(nely, nelx);
            Drto_constraint2 = 0 * Drto_constraint;

            v_mean_fraction = sum(sum(rho_Phys)) / (nelx * nely * volfrac);
        ...
        % Density filtering
        Drto_obj(:) = H * (Drto_obj(:) ./ Hs);
        Drto_constraint(:) = H * (Drto_constraint(:) ./ Hs);
        ...
        % Apply density filter on design variable vector
        rho_Phys = (H * rho(:)) ./ Hs;
        rho_Phys = reshape(rho_Phys, nely, nelx);

        % Record iteration history of physical density
